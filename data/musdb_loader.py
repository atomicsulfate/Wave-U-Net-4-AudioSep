...
def get_musdbhq(database_path):
    ...
    for subset in ["train", "test"]:
        logger.info("Loading " + subset + " set...")
        ...
        for track_folder in sorted(tracks):
            ...
            if not os.path.exists(acc_path):
                logger.debug("Writing accompaniment to " + track_folder)
                stem_audio = []
    ...
    for subset in ["train", "test"]:
        ...
        for track in tracks:
            ...
            if os.path.exists(mix_path):
                logger.debug(f"Skipping {subset} track {mix_path} since it exists already")
            ...
            logger.debug("Maximum absolute deviation from source additivity constraint: " + str(np.max(diff_signal)))# Check if acc+vocals=mix
            logger.debug("Mean absolute deviation from source additivity constraint:    " + str(np.mean(diff_signal)))
...
def get_musdb_folds(root_path, version="HQ"):
    ...
    # TODO: when running on cluster, change replace=True to False
    train_list = np.random.choice(train_val_list, 75, replace=True)
